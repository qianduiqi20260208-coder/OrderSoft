...
function renderChart() {
  ...
  try {
    ...
    if (selectedTimePeriod.value === '7days') {
      currentData = chartStatistics.value.daily.filter(item =>
        selectedClients.value.includes(item.clientName),
      )
      // 按日期分组
      ...
      currentData = chartStatistics.value.weekly.filter(item =>
        selectedClients.value.includes(item.clientName),
      )
      // 修改：按周标签分组
      const weekGroups = ['第1周', '第2周', '第3周', '第4周(本周)']
      xAxisData = weekGroups
    }
    else {
      currentData = chartStatistics.value.monthly.filter(item =>
        selectedClients.value.includes(item.clientName),
      )
      // 修改：按月标签分组
    ...
    if (selectedTimePeriod.value === '7days') {
      ...
      dateGroups.forEach((date) => {
        selectedClients.value.forEach((client) => {
          ...
          const count = selectedOrderType.value === '版本迭代'
            ? (item?.versionIterationCount || 0)
            : (item?.deliveryCount || 0)
          clientGroups[client].push(count)
        })
      })
    }
    else if (selectedTimePeriod.value === '1month') {
    ...
    const series = selectedClients.value.map((clientName, index) => ({
      name: clientName,
      type: 'line',
      smooth: false,
      symbol: 'circle',
