...
// 新增：修改密码事件处理
async function onChangePassword(data: { account: string, oldPassword: string, newPassword: string }) {
  try {
    console.warn('修改密码数据:', data)
...
// 新增：处理返回登录事件
function onBackToLogin(accountValue?: string) {
  formType.value = 'login'
  if (accountValue) {
