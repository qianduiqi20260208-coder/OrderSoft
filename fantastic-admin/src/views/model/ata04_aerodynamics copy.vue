...
function openFunctionDialog(model: Model) {
  ...
  currentFunctionOrders.value = model.orders.filter(o => o.type === 'function')
...
function openReproduceDialog(model: Model) {
  currentModel.value = model
  currentReproduceOrders.value = model.orders.filter(o => o.type === 'reproduce')
...
function openClientDialog(model: Model) {
  currentModel.value = model
  currentDeliveryOrders.value = model.orders.filter(o => o.type === 'delivery')
...
function openCoordinationOrderDialog(model: Model) {
  currentModel.value = model
  currentCoordinationOrders.value = model.orders.filter(o => o.type === 'iteration' && o.coordinationSheet)
...
async function apiFetchUserModels() {
  ...
  return res.data || { list: [], total: 0 } // 返回数据或默认空数据
}
...
async function fetchUserModels() {
  ...
  try {
    ...
    modelList.value = result.list || [] // 设置模型列表
        ...
        >
          <template #title>
            <div class="flex items-center gap-2">
              <el-tooltip
                :content="model.orders.some(o => o.type === 'iteration' && o.apiChanged) ? '接口与父版本有变化' : '接口与父版本无变化'"
                placement="top"
              >
                <FaIcon
                  :name="model.orders.some(o => o.type === 'iteration' && o.apiChanged) ? 'error_state' : 'ready_state'"
                  :class="model.orders.some(o => o.type === 'iteration' && o.apiChanged) ? 'size-8 text-red-500' : 'size-8 text-green-500'"
                />
              </el-tooltip>
              <span class="ml-2 text-2xl font-bold">ATA04 Aerodynamics
                <sup
                  class="ml-1 text-sm font-bold"
                  :class="model.orders.some(o => o.type === 'delivery' && o.isCAEChecked) ? 'text-green-700' : 'text-gray-400'"
                  style="line-height: 1;"
                >
                  CAE
                  <FaIcon
                    v-if="model.orders.some(o => o.type === 'delivery' && o.isCAEChecked)"
                    name="check"
              ...
              <div class="ml-auto flex gap-2">
                <FaButton
                  ...
                  :class="[
                    model.orders.some(o => o.type === 'function')
                  ...
                  :disabled="!model.orders.some(o => o.type === 'function')"
                ...
                <FaButton
                  class="rounded px-5 py-1.5 font-semibold transition-colors duration-150"
                  :class="[
                    model.orders.some(o => o.type === 'reproduce')
                      ? 'bg-purple-300 text-black border border-purple-500 hover:bg-purple-400'
                      : 'bg-gray-300 text-gray-400 border border-gray-400 cursor-not-allowed',
                  ]"
                  :disabled="!model.orders.some(o => o.type === 'reproduce')"
                ...
                <FaButton
                  class="rounded px-5 py-1.5 font-semibold transition-colors duration-150"
                  :class="[
                    model.orders.some(o => o.type === 'iteration' && o.coordinationSheet)
                      ? 'bg-blue-300 text-black border border-blue-500 hover:bg-blue-400'
                      : 'bg-gray-300 text-gray-400 border border-gray-400 cursor-not-allowed',
                  ]"
                  :disabled="!model.orders.some(o => o.type === 'iteration' && o.coordinationSheet)"
                ...
                <FaButton
                  class="rounded px-5 py-1.5 font-semibold transition-colors duration-150"
                  :class="[
                    model.orders.some(o => o.type === 'delivery')
                      ? 'bg-yellow-300 text-black border border-yellow-500 hover:bg-yellow-400'
                      : 'bg-gray-300 text-gray-400 border border-gray-400 cursor-not-allowed',
                  ]"
                  :disabled="!model.orders.some(o => o.type === 'delivery')"
            ...
            <div class="ml-2 mt-2 flex items-center gap-2">
              <span class="text-base text-black">版本号：{{ model.version }}</span>
          ...
          <template #collapse-summary>
            <div class="flex items-center text-black">
              {{ model.version }}
              <FaIcon name="updatetime" class="mx-1 size-8" />
              {{ model.updatetime }}
            </div>
          </template>
          <!-- 动态渲染该模型下所有版本迭代类工单 -->
          <div class="pl-4 space-y-4">
            <FaPageMain
              v-for="order in model.orders.filter(o => o.type === 'iteration')"
              :key="order.orderID"
            ...
            >
              ...
              <div class="grid grid-cols-2 items-start gap-x-8 gap-y-4">
                ...
                <!-- 发起日期 -->
                <div class="col-span-1 w-full flex items-center gap-2">
                  <span class="w-32 text-black font-semibold">发起日期：</span>
                  <input
                    class="flex-1 border border-gray-200 rounded bg-gray-50 px-3 py-2 text-sm text-blue-700 font-bold"
                ...
                <!-- 相应协调单 -->
                <div class="col-span-1 w-full flex items-center gap-2">
                  <span class="w-32 text-black font-semibold">相应协调单：</span>
                  <input
                    class="flex-1 border border-gray-200 rounded bg-gray-50 px-3 py-2 text-sm text-blue-700 font-bold"
                    :value="order.coordinationSheet ? '是' : '否'"
                    readonly
                  >
                </div>
                <!-- 接口变化 -->
                ...
                <!-- 完成人 -->
                <div class="col-span-1 w-full flex items-center gap-2">
                  <span class="w-32 text-black font-semibold">完成人：</span>
                  <input
                    class="flex-1 border border-gray-200 rounded bg-gray-50 px-3 py-2 text-sm text-blue-700 font-bold"
                    :value="order.finisherID"
                    readonly
                  >
                </div>
                <!-- 完成时间 -->
              ...
              <template v-for="order in currentDeliveryOrders" :key="order.orderID">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <input
                    ...
                    :class="order.isCAEChecked ? 'text-blue-700' : 'text-green-700'"
                ...
                <div class="flex items-center gap-2">
                  ...
                  <input
                    class="flex-1 border border-gray-200 rounded bg-gray-50 px-3 py-2 font-bold"
                    :class="order.isEncrypted ? 'text-blue-700' : 'text-green-700'"
