...
const totalSendCount = ref(0) // 总发送数量
const totalModelCount = ref(0) // 总模型数量
const totalModelVersionCount = ref(0) // 总模型版本数量
...
// 发送总览处理函数
function handleSendOverview() {
  overviewDialogVisible.value = true
}

// 版本比较函数：根据新的版本格式进行排序
function compareVersions(versionA: string, versionB: string): number {
  // 解析版本号格式：V主版本.次版本.模型版本.封装版本平台标识符
  // 例如：V2.0.1.1C
  const parseVersion = (version: string) => {
    // 移除 V 前缀
    const withoutV = version.substring(1)

    // 提取平台标识符（最后一个字符）
    const platform = withoutV.slice(-1)

    // 提取数字部分（去掉平台标识符）
    const numbersStr = withoutV.slice(0, -1)

    // 分割版本号数字
    const numbers = numbersStr.split('.').map(num => Number.parseInt(num, 10))

    return {
      major: numbers[0] || 0, // 主版本号
      minor: numbers[1] || 0, // 次版本号
      model: numbers[2] || 0, // 模型版本号
      package: numbers[3] || 0, // 封装版本号
      platform: platform || 'A', // 平台标识符
    }
  }

  const versionAObj = parseVersion(versionA)
  const versionBObj = parseVersion(versionB)

  // 按照优先级比较：主版本 > 次版本 > 模型版本 > 封装版本 > 平台标识符
  if (versionAObj.major !== versionBObj.major) {
    return versionBObj.major - versionAObj.major // 倒序：较新版本在前
  }

  if (versionAObj.minor !== versionBObj.minor) {
    return versionBObj.minor - versionAObj.minor
  }

  if (versionAObj.model !== versionBObj.model) {
    return versionBObj.model - versionAObj.model
  }

  if (versionAObj.package !== versionBObj.package) {
    return versionBObj.package - versionAObj.package
  }

  // 平台标识符按字母顺序比较，A < B < C < D...
  if (versionAObj.platform !== versionBObj.platform) {
    return versionBObj.platform.localeCompare(versionAObj.platform)
  }
...
// 获取每个模型的最新版本记录
function getLatestVersionRecords(): SendRecord[] {
  if (!sendDetail.value) {
    return []
  }

  // 收集所有发送记录
  const allRecords: SendRecord[] = []
  sendDetail.value.dailySendRecords.forEach((dailyRecord) => {
    allRecords.push(...dailyRecord.records)
  })

  // 按模型名称分组
  const modelGroups = new Map<string, SendRecord[]>()
  allRecords.forEach((record) => {
    if (!modelGroups.has(record.modelId)) {
      modelGroups.set(record.modelId, [])
    }
    modelGroups.get(record.modelId)!.push(record)
  })

  // 获取每个模型的最新版本记录
  const latestRecords: SendRecord[] = []
  modelGroups.forEach((records) => {
    // 首先按版本号排序，取最高版本
    const sortedByVersion = records.sort((a, b) => {
      return compareVersions(a.modelVersionId, b.modelVersionId)
    })

    // 如果有多个相同的最高版本，取时间最新的
    const latestVersion = sortedByVersion[0].modelVersionId
    const sameVersionRecords = sortedByVersion.filter(r => r.modelVersionId === latestVersion)

    const sortedByTime = sameVersionRecords.sort((a, b) => {
      const timeA = new Date(a.sendTime).getTime()
      const timeB = new Date(b.sendTime).getTime()
      return timeB - timeA // 倒序：最新时间在前
    })

    // 取最新的记录
    if (sortedByTime.length > 0) {
      latestRecords.push(sortedByTime[0])
    }
  })

  // 按模型名称排序，保证显示顺序一致
  return latestRecords.sort((a, b) => a.modelId.localeCompare(b.modelId))
}
  ...
  try {
    ...
    if (res?.data) {
      ...
      const mappedData: SendDetail = {
        ...
        modelCount: res.data.modelCount || 0, // 发送模型数量
        modelVersionCount: res.data.modelVersionCount || 0, // 发送模型版本数量
        // 按日期分组的发送记录（当前页的数据）
      ...
      totalSendCount.value = res.data.totalSendCount || 0 // 总发送数量
      totalModelCount.value = res.data.totalModelCount || 0 // 总模型数量
      totalModelVersionCount.value = res.data.totalModelVersionCount || 0 // 总模型版本数量
      ...
      console.warn('获取发送详情成功:', {
        ...
        totalSendCount: totalSendCount.value,
      })
    }
    else {
          ...
          <div class="mt-3">
            ...
            <div v-if="sendDetail" class="grid grid-cols-2 gap-6">
              <div class="flex items-center gap-3">
                ...
                <span class="text-2xl text-blue-600 font-bold">{{ sendDetail.modelCount }}</span>
              </div>
              <div class="flex items-center gap-3">
                <span class="text-sm text-gray-600">发送模型版本数量：</span>
                <span class="text-2xl text-green-600 font-bold">{{ sendDetail.modelVersionCount }}</span>
              </div>
            </div>

            <!-- 分页信息提示 -->
            <div class="mt-4 text-sm text-gray-500">
              当前显示第 {{ currentPage }} 页，共 {{ Math.ceil(total / pageSize) }} 页，每页显示 {{ pageSize }} 条日期记录
            </div>
          </div>
        </div>
        ...
        >
          <template #header>
            <div class="w-full flex items-center justify-between">
              <span class="text-xl font-bold">发送总览 - 各模型最新版本</span>
              <span class="text-lg text-blue-600 font-bold">
                模型数量：{{ getLatestVersionRecords().length }} / {{ sendDetail?.modelCount || 0 }}
              </span>
            </div>
          </template>
          ...
          <div class="overflow-y-auto space-y-4" style="height: 580px; max-height: 80vh;">
            <!-- 如果有最新版本记录 -->
            <div v-if="getLatestVersionRecords().length > 0">
              <div
                v-for="(record, index) in getLatestVersionRecords()"
                :key="record.orderId"
              >
                <div class="border border-gray-200 rounded-lg bg-gray-50 p-4">
                  <div class="grid grid-cols-2 items-start gap-x-8 gap-y-4">
                    ...
                    <!-- 第二行：版本号和更新时间 -->
                    <div class="flex items-center gap-2">
                      <span class="w-24 text-black font-semibold">最新版本：</span>
                      <input
                        class="flex-1 border border-gray-200 rounded bg-gray-50 px-3 py-2 text-sm text-green-700 font-bold"
                        :value="record.modelVersionId"
                        readonly
                    ...
                    <div class="flex items-center gap-2">
                      <span class="w-24 text-black font-semibold">更新时间：</span>
                      <input
                ...
                <!-- 分割线，除了最后一条记录 -->
                <div
                  v-if="index < getLatestVersionRecords().length - 1"
