...
const routes: RouteRecordRaw = {
  path: '/ata7x_engine', // 一级路由路径
  component: Layout,
  name: 'ATA7x_Engine', // 路由名称，唯一标识
  meta: {
  ...
  children: [
    {
      ...
      name: 'ATA7x_Engine_Info', // 子路由名称，唯一标识 这里不重要
      component: () => import('@/views/model/model_list.vue'), // 具体页面组件
      meta: {
        title: 'ATA7x_发动机模型', // 页面标题
        menu: false, // 不在菜单中显示
        breadcrumb: false, // 不在面包屑中显示
        activeMenu: '/ata7x_engine', // 激活菜单项（高亮显示) 设置为一级路由路径即可
      },
    },
  ],
}
