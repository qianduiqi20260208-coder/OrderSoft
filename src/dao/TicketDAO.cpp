...
std::vector<nlohmann::json> TicketDAO::getWorkOrdersWithDetailsByVersions(const std::string& modelName, const std::vector<std::string>& versions)
{
    ...
    ss << "("
       ...
       << "mv.version, "
       << "mv.update_time, "
       << "CONCAT( wo.creator_id, CASE WHEN u1.real_name IS NOT NULL AND u1.real_name != '' THEN CONCAT('(', u1.real_name, ')') ELSE '' END ) AS creatorId, "
       ...
       << "wo.status, "
       << "CONCAT( wo.approver_id, CASE WHEN u2.real_name IS NOT NULL AND u2.real_name != '' THEN CONCAT('(', u2.real_name, ')') ELSE '' END ) AS approverId, "
       << "wo.priority as priorityHint, "
       << "CONCAT( wo.dispatcher_id, CASE WHEN u3.real_name IS NOT NULL AND u3.real_name != '' THEN CONCAT('(', u3.real_name, ')') ELSE '' END ) AS distributorId, "
       << "CONCAT( woe.executor_id, CASE WHEN u4.real_name IS NOT NULL AND u4.real_name != '' THEN CONCAT('(', u4.real_name, ')') ELSE '' END ) AS executorID, "
