...
void CustomerInfoController::registerRoutes(crow::SimpleApp& app) {
    ...
    CROW_ROUTE(app, "/dongle/create").methods("POST"_method)
        ...
        try {
            ...
            // TODO: 调用服务层创建加密狗
            // bool result = dongleService->createDongle(shellCode, shellSerial);
    ...
    CROW_ROUTE(app, "/delivery/dongles/update").methods("PUT"_method)
        ...
        try {
            ...
            printf("[DEBUG] 更新加密狗信息，dongleId: %s, shellCode: %s, shellSerial: %s\n", 
                   dongleId.c_str(), shellCode.c_str(), shellSerial.c_str());

            // TODO: 调用服务层更新加密狗信息
            // bool result = dongleService->updateDongle(dongleId, shellCode, shellSerial);
